...
async function actualizarDatos(moneda) {
  try {
    ...
    if (datos) {
      ...
      document.getElementById(
        "precioVenta"
      ).innerText = `(Venta: ${datos.precioVenta.toFixed(2)} Bs/$US)`;
...
async function buscarCotizaciones(fechaInicio, fechaFin, moneda) {
  try {
    const response = await fetch(
      `http://localhost:3000/api/cotizaciones?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&moneda=${moneda}`
    );
...
function crearGrafico(canvas, datos, etiquetas, preciosVenta, etiqueta) {
  ...
  const promedio =
    preciosVenta.reduce((sum, value) => sum + value, 0) / preciosVenta.length;
      ...
      datasets: [
        {
          ...
          pointBackgroundColor: (context) => {
            ...
            return preciosVenta[index] > preciosVenta[index - 1]
              ? "#4CAF50"
              : "#f44336";
          },
          fill: true,
          ...
          callbacks: {
            label: function (context) {
              ...
              return [
                ...
                `Cambio: ${change >= 0 ? "+" : ""}${change.toFixed(
                  4
...
function graficarDatos(moneda) {
  ...
  let datosLocalStorage =
    JSON.parse(localStorage.getItem(`precios_${moneda}`)) || [];
  ...
  fetch("data-bs-binance.xml")
    ...
    .then((jsonText) => {
      ...
      window.datosValidos = Array.from(mapaRegistros.values()).sort(
...
function filtrarYMostrarDatos(periodo) {
    ...
    case "1d":
      ...
      datosUltimoDia.forEach((d) => {
        ...
        const clave =
          ...
          fecha.getHours() +
          ":" +
          (Math.floor(fecha.getMinutes() / 15) * 15)
            .toString()
            .padStart(2, "0");
  ...
  if (datosFiltrados.length > 2) {
    ...
    for (const [hora, valores] of Object.entries(volatilidadPorHora)) {
      if (valores.length > 1) {
        ...
        const varianza =
          valores.reduce((a, v) => a + Math.pow(v - prom, 2), 0) /
          valores.length;
    ...
    if (horaMax !== null) {
      volatilidadHora =
        horaMax.padStart(2, "0") + ":00-" + horaMax.padStart(2, "0") + ":59";
    }
  }
  ...
  if (canvas) {
    ...
    window.graficoActual = crearGrafico(
      canvas,
      datosFiltrados,
      fechas,
      preciosVenta,
...
function configurarBotonCompartir(datosValidos) {
  document
    ...
    .addEventListener("click", async () => {
      ...
      const mensaje = `Cotización del dólar: ${ultimoDato.precioVenta.toFixed(
        2
      )} Bs/$US\nFecha: ${new Date(
        ultimoDato.fechaHora
...
function actualizarEstadisticas(moneda, datos) {
  ...
  const spreadActual =
    preciosVenta[preciosVenta.length - 1] -
    preciosCompra[preciosCompra.length - 1];
  const spreadPorcentaje =
    (spreadActual / preciosCompra[preciosCompra.length - 1]) * 100;

  const max = Math.max(...preciosVenta);
  const min = Math.min(...preciosVenta);
  const promedio =
    preciosVenta.reduce((sum, val) => sum + val, 0) / preciosVenta.length;

  // Calcular volatilidad (desviación estándar)
  const varianza =
    preciosVenta.reduce((sum, val) => sum + Math.pow(val - promedio, 2), 0) /
    preciosVenta.length;
  ...
  const estadisticasHTML = `
        ...
        <div class="stat-grid">
            <div class="stat-item">
                ...
                <span class="stat-value">${precioActual.toFixed(
                  4
                )} Bs/$US</span>
            </div>
            <div class="stat-item">
                ...
                <span class="stat-value ${
                  cambio >= 0 ? "positivo" : "negativo"
                }">${cambio.toFixed(4)} Bs/$US (${cambioPorcentaje.toFixed(
    2
  )}%)</span>
            </div>
            <div class="stat-item">
                <span class="stat-label">Spread (Venta-Compra):</span>
                <span class="stat-value">${spreadActual.toFixed(
                  4
                )} Bs/$US (${spreadPorcentaje.toFixed(2)}%)</span>
            </div>
            <div class="stat-item">
...
function updateChartByPeriod(period) {
  ...
  if (startDate) {
    filteredData = window.datosValidos.filter(
      (item) => new Date(item.fecha) >= startDate
    );
  } else {
    filteredData = window.datosValidos; // Usar todos los datos si se selecciona 'all'
  }
...
function actualizarGrafico(datosAMostrar) {
  try {
    ...
    mostrarMensajeGrafico(
      "Error al cargar el gráfico. Intenta recargar la página."
    );
  }
}

// Función para mostrar mensajes de error en el gráfico
function mostrarMensajeGrafico(mensaje) {
...
function conectarWebSocket() {
  // Solo intentar conexión si estamos en un entorno que soporta WebSocket real
  if (
    window.location.protocol === "file:" ||
    window.location.hostname === "jvejarano.github.io"
  ) {
    console.log(
      "WebSocket no disponible en este entorno (GitHub Pages o archivo local)"
    );
    return;
  }
  ...
  if (reconnectAttempts >= MAX_RECONNECTS) {
    console.log(
      `Máximo de ${MAX_RECONNECTS} intentos de reconexión alcanzado. Deteniendo intentos.`
    );
    return;
  }
  ...
  try {
    ...
    ws.onclose = (event) => {
      if (event.code !== 1000) {
        ...
        console.log(
          `Conexión WebSocket cerrada (código: ${event.code}). Intento ${
            reconnectAttempts + 1
          }/${MAX_RECONNECTS}`
        );
        reconnectAttempts++;
...
function actualizarDatosEnTiempoReal(nuevaCotizacion) {
  ...
  // Ordenar por fecha
  window.datosValidos.sort(
    (a, b) => new Date(a.fechaHora) - new Date(b.fechaHora)
  );

  // Actualizar gráfico con animación
  const periodoActual =
    document.querySelector(".period-btn.active")?.dataset.period || "1w";
...
function actualizarBrecha() {
  const ventaUSDT = parseFloat(
    document
      .getElementById("precioVentaBinance")
      .textContent.replace(/[^\d.]/g, "")
  );
  const ventaOficial = parseFloat(
    document
      .getElementById("precioVentaOficial")
      .textContent.replace(/[^\d.]/g, "")
  );
  ...
  brechaEl.textContent = `${brecha >= 0 ? "+" : ""}${brecha.toFixed(2)} Bs (${
    brechaPorc >= 0 ? "+" : ""
  }${brechaPorc.toFixed(2)}%)`;
...
function hookBrecha() {
  ...
  obs.observe(document.getElementById("precioVentaBinance"), {
    childList: true,
  });
  obs.observe(document.getElementById("precioVentaOficial"), {
    childList: true,
  });
}
...
function calcularConversion() {
  ...
  const precioVentaBinance = parseFloat(
    document.getElementById("precioVentaBinance").textContent.replace("$", "")
  );
...
document.addEventListener("DOMContentLoaded", () => {
  // Solo intentar WebSocket si estamos en un entorno compatible
  if (
    window.location.protocol !== "file:" &&
    window.location.hostname !== "jvejarano.github.io"
  ) {
    ...
    console.log("WebSocket deshabilitado para GitHub Pages y archivos locales");
  }
  ...
  document
    ...
    ?.addEventListener("click", calcularConversion);
  document
    .getElementById("montoCalculadora")

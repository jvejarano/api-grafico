// Este archivo está diseñado para ejecutarse en Node.js, no en el navegador
// Si se incluye accidentalmente en el cliente, evitar errores
if (typeof window === "undefined") {
  ...
  // Entorno navegador - no ejecutar este código
  console.warn(
  ...
  try {
        ...
        .on("data", (row) => {
          try {
            ...
            const data = {
              ...
              precioCompraOficial: parseFloat(
                row.compraOficial.replace(",", ".")
              ),
              precioVentaOficial: parseFloat(
                row.ventaOficial.replace(",", ".")
              ),
              precioCompraBinance: parseFloat(
                row.compraBinance.replace(",", ".")
              ),
              precioVentaBinance: parseFloat(
                row.ventaBinance.replace(",", ".")
              ),
            };
            // Solo agregar si los valores son válidos
            if (
              !isNaN(data.precioCompraBinance) &&
              !isNaN(data.precioVentaBinance)
            ) {
              csvData.push(data);
            }
          } catch (e) {
    ...
    try {
      ...
      if (
        ...
        localData = JSON.parse(localStorage.getItem(localStorageKey)).map(
          (item) => ({
            ...item,
            source: "localStorage",
          })
    ...
    fs.writeFileSync("data-combined.json", JSON.stringify(uniqueData, null, 2));

    console.log(
      `Combinación completada. Total de registros: ${uniqueData.length}`
    );
    return uniqueData;
  } catch (error) {
